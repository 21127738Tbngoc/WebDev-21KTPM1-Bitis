...
router.get('/all-orders', async function (req, res, next) {
    ...
    let allOrders = await axios.get('http://localhost:3000/order/').then(res => res.data)
...
router.get('/all-products', async function (req, res, next) {
    ...
    let Data = await axios.get('http://localhost:3000/product/').then(res => res.data)
    res.render('pages/admin/product.hbs', { title: 'Quản lí sản phẩm', layout: 'admin.hbs', productsData: Data,  });
});
